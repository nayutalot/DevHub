...
if (isEntrypoint()) {
  ...
  registerCase('step3: fresh db migrates to user_version 1, idempotent re-run', async () => {
    ...
    try {
      ...
      assert.equal(applied, 4, '001 + 002 + 003 + 004 migrations applied on fresh db (AC2 就地更新 3→4)')
      const row = db.prepare('PRAGMA user_version').get()
      assert.equal(Number(row.user_version), 4, 'user_version after migrate (latest = 4, AC2 就地更新 3→4)')
  ...
  registerCase('step5: migration 002 relaxes env tool uniqueness to (env,tool,path); v1 db upgrades with data intact', async () => {
    ...
    try {
      const applied = dbModule.migrate(db)
      assert.equal(applied, 4, '001+002+003+004 applied on fresh db (AC2 就地更新 3→4)')
      assert.equal(Number(db.prepare('PRAGMA user_version').get().user_version), 4, 'fresh db at user_version 4 (AC2 就地更新 3→4)')
    ...
    try {
      ...
      assert.equal(applied, 3, 'only 002, 003 and 004 apply to the v1 library (AC2 就地更新 2→3)')
      assert.equal(Number(db2.prepare('PRAGMA user_version').get().user_version), 4, 'v1 upgraded to user_version 4 (AC2 就地更新 3→4)')
  ...
  registerCase('step5: scanService persists fixture projects, dirty repository, scans row and contains edge', async () => {
    ...
    try {
      ...
      assert.equal(Number(db.prepare('PRAGMA user_version').get().user_version), 4, 'home db migrated to 4 (AC2 就地更新 3→4)')
  ...
  registerCase('s1-40: migration 003 — fresh db at user_version 3 with merge tables; v2 db upgrades preserving data', async () => {
    ...
    try {
      ...
      const applied = dbModule.migrate(db2)
      assert.equal(applied, 2, 'only 003 and 004 apply to the v2 library (AC2 就地更新 1→2)')
      assert.equal(Number(db2.prepare('PRAGMA user_version').get().user_version), 4, 'v2 upgraded to user_version 4 (AC2 就地更新 3→4)')
  ...
  registerCase('s1-44: real-db readonly assertion — skill_agents >= 1 and archive_runs >= 1 after real import', async () => {
    ...
    try {
      ...
      assert.equal(migrated, 4, `real db at user_version 4, got ${migrated} (AC2 就地更新 3→4)`)
  ...
  registerCase('ac2-81: migration 004 fresh — user_version=4, 8 new tables, 4 settings seeds, agent_events AUTOINCREMENT sequence', async () => {
    ...
    try {
      const applied = dbModule.migrate(db)
      assert.equal(applied, 4, '001..004 applied on fresh db')
      assert.equal(Number(db.prepare('PRAGMA user_version').get().user_version), 4, 'fresh db at user_version 4')
  ...
  registerCase('ac2-82: migration 004 v3→v4 upgrade — legacy 19 tables row-for-row unchanged (T1), only 004 applies, 4 settings seeds added', async () => {
    ...
    try {
      ...
      const applied = dbModule.migrate(db2)
      assert.equal(applied, 1, 'only 004 applies to the v3 library')
      assert.equal(Number(db2.prepare('PRAGMA user_version').get().user_version), 4, 'v3 upgraded to user_version 4')
