...
if (isEntrypoint()) {
  ...
  // smoke fake transport 零联网）。
  // ------------------------------------------------------------------
  registerCase('step1: channels whitelist has exactly 92 entries (LR1 就地更新 88→92) and IPC_GATEWAY', async () => {
    ...
    assert.equal(channels.IPC_CHANNELS.length, 92, `expected 92 channels, got ${channels.IPC_CHANNELS.length}`)
    assert.deepEqual([...channels.IPC_CHANNELS], expected, 'whitelist must match docs/04 + docs/09 §9 + docs/10 §11 + docs/14 §A.1 + docs/04 ContestPin 追加节 + docs/22 §4/§6 + docs/04 LR1 追加节 exactly')
    assert.equal(new Set(channels.IPC_CHANNELS).size, 92, 'no duplicate channels')
  }, 'fast')
  ...
  // ------------------------------------------------------------------
  registerCase(
    'step6: handler registry keys equal the 92-channel whitelist (LR1 就地更新 88→92); app:version returns injected value; unknown channel folds to CHANNEL_NOT_ALLOWED envelope',
    async () => {
      const channels = await import(new URL('../src/shared/channels.ts', import.meta.url).href)
      ...
      assert.equal(keys.length, 92, `registry must hold exactly 92 handlers, got ${keys.length}`)
  ...
  //  识别配置 4 条；LR1 就地更新 88→92，docs/04「LR1 追加」节 LLM 复核层 4 条）：
  //  registry 键集 = 白名单 = 契约覆盖
  registerCase('s4-68: whitelist 45→50 (S5 就地更新为 55，AC2 就地更新 55→68，夜间#1 就地更新 68→70，CP1 就地更新 70→79，CP2 就地更新 79→84，CP3a 就地更新 84→88，LR1 就地更新 88→92) — registry keys equal the whitelist and the compile-time contract assertion holds', async () => {
    const channels = await import(new URL('../src/shared/channels.ts', import.meta.url).href)
    const handlers = await import(new URL('../src/main/ipc/handlers.ts', import.meta.url).href)

    assert.equal(channels.IPC_CHANNELS.length, 92, 'whitelist extended 45 → 50 (S4), 50 → 55 (S5 archive), 55 → 68 (AC2 agents), 68 → 70 (夜间#1), 70 → 79 (CP1 contestpin 9 条), 79 → 84 (CP2 contestpin 悬浮窗 5 条), 84 → 88 (CP3a contestpin 识别配置 4 条), 88 → 92 (LR1 LLM 复核层 4 条)')
    assert.equal(new Set(channels.IPC_CHANNELS).size, 92, 'no duplicates after extension')
  ...
  registerCase('ac2-84: agents channels (14, 夜间#1 就地更新 13→14) — whitelist tail in docs/14 §A.1 order, registry handlers, compile-time contract assertion holds（LR1 就地更新 88→92：LLM 复核层 4 条尾窗）', async () => {
    const channels = await import(new URL('../src/shared/channels.ts', import.meta.url).href)
    const handlers = await import(new URL('../src/main/ipc/handlers.ts', import.meta.url).href)
    ...
    assert.equal(channels.IPC_CHANNELS.length, 92, 'whitelist 55 → 70 (docs/14 §A.2; 夜间#1 就地更新 68→70), 70 → 79 (CP1 就地更新，docs/04 ContestPin 追加节), 79 → 84 (CP2 就地更新，docs/22 §4 悬浮窗 5 条), 84 → 88 (CP3a 就地更新，docs/22 §6 识别配置 4 条), 88 → 92 (LR1 就地更新，docs/04 LR1 追加节 LLM 复核层 4 条)')
    // CP3a 就地更新：CP2 后追加 CP3a contestpin 4 条（configList/Save/Delete/Test）；
    // LR1 就地更新：白名单尾部再追加 LLM 复核层 4 条，agents 尾窗再前移为 slice(-36, -22)
    assert.deepEqual([...channels.IPC_CHANNELS.slice(-36, -22)], expectedAgents, '14 agents channels appended verbatim in docs/14 §A.1 order (夜间#1 就地更新 13→14)')
    assert.deepEqual(
    ...
    assert.deepEqual(
      [...channels.IPC_CHANNELS.slice(-13, -8)],
      [
        'contestpin:overlayState',
        'contestpin:overlaySetEnabled',
        'contestpin:overlaySetCollapsed',
