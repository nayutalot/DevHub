# DevHub 会话交接文档（2026-09-16 凌晨——UX-Z3 对话 v2 运行态层（计时器+工具活动 pill+终态人话）；main 见 git log，可验收 APK dc590461）

> **✅ UX-Z3 对话 v2 运行态层批（09-16 凌晨，docs/briefs/uxz3-runtime.md + docs/28 §6 运行态节落地）——四件全收，可验收 APK dc590461（覆盖 dist）**：**①会话运行计时器 E16**（会话详情运行状态条「已工作 X 分 Y 秒」每秒走字——running 态区间=status_changed to:'running' 起、五终态{waiting_input,completed,failed,paused,connection_lost}停停沿冻结实测区间；:core RunTurnClock 纯函数+SessionRunRegistry 进程内锚点登记〔ConnectionManager local/relay 事件帧+详情轮询双喂、沿去重、**首观测即 running/断线/重启锚点不可考→降级「运行中…」绝不伪造**〕；observed 整块不显）；**②工具活动 pill E19a 收紧版**（:core ToolActivityPill——**白名单先真实样本实测后定**：zcode 真库 tool_usage 37,273 行/52 工具名全集+投影面 16,874 段实查〔全挂 role='assistant' 行〕+deepseek firehose 真机 e2e 双行投影〔assistant 折叠行+role='tool' 行→载体裁定只按 role='tool' 计消双写重复〕；文件白名单 Read/Edit/Write/MultiEdit/NotebookEdit/apply_patch，Bash/PowerShell 判定不了不入表绝不猜；running「已进行 N 次文件操作/运行中 · 第 N 步」、停沿转场「本轮完成 · N 次操作/本轮出错结束/本轮已取消/本轮已结束」；无 tool 行/无锚点不画数字）；**③E21 运行中 composer**（running 期间保持可用+占位「提出后续修改要求」v4 形态，既有 reply 通道/ControlGate 逐字节不动）；**④statusDetail 终态族补全**（zcode managed「turn completed (resultType: …)」七值+「zcode event: <type>」六型入 core.StatusDetailHumanize 词表，表外零吞码）。走查 9 张 headless 模拟器实拍=acceptance/uxz3-walkthrough/（真实事件管道：夹具网关 18790+回环控制面经 applySessionStatus/persistMessage 同桌面 managed 面落库+WS 推送非 DB 摆拍；计时器±1s/降级路径/observed 双门/pill 与消息流 tool 行数对照；用后 taskkill+emu kill 零常驻，凭据三零）。门禁 :app **194**（184+10 registry 锁）/:core **361**（343+18 真实样本锁）全绿+assembleDebug。**APK dc590461477927c1f361669da892307056e404e63c1791d3e05282a164bd8355=最新待装件**（dist 根其余未动）。走查实证缺陷已修：停沿快照曾丢 anchor→转场 pill 窗口不可归属（05f9835）。

> **✅ UX-Z2 对话 v2 结构层批（09-16 凌晨，docs/briefs/uxz2-structure.md + docs/28 §4/§5 落地）——四件全收，可验收 APK 9b27d2b6（覆盖 dist）**：**①桌面只读聚合投影**（SessionView 纯追加 `workdir`+providerView 纯追加 `managedModel`=zcode/deepseek 设置键直投〔展示不承诺切换，kimi 无模型概念恒缺省〕+`agents:sessionWorkspaces` IPC/GW `GET /v1/sessions/workspaces` GROUP BY workdir 聚合〔归一键/尾段名/未分组尾部/归档排除〕；白名单计数就地更新 115→116，smoke 新增 uxz2-w1/w2 既有零删改）；**②Android 段控**「最近对话｜工作区」（E1 汇总行/E2 工作区卡/E3 「更新于 X」相对时间/chevron 展开任务行/「＋ 新对话」；分组=:core WorkspaceGrouping 纯函数与桌面同口径单测锁定；WebView 遥控置顶卡并存；Room v5→v6 session_cache+workdir 增量迁移 MIGRATION_5_6）；**③composer-first 新建流**（P3 升级迁移至对话 tab NewChatScreen：E4 时段问候语无称呼不伪造/E8 chips 四枚 DevHub 语境点击仅填入/E10 模型弹层诚实三态〔可选面只读+候选灰显+电脑指引/托管停用态/无 managed 不画入口〕/E6 工作区只读展示；助手页「开始对话」改跳转 autoOpenComposer 一次性语义；提交路径/门语义逐字节保留）；**④杂项**（TimeFmt.rel 与桌面 relativeTime 中文口径逐字一致+MessageBubble 复制钮〔赞/踩 C 档不画〕+statusDetail「turn/end (seq N)」人话化收口 core.StatusDetailHumanize 未命中零吞码）。红线全保：C 档零出现/U1-U5、P1-P3 不回退/ControlGate 语义不动/深链零破坏（devhub://session/4 走查实证）。门禁 typecheck 0+fast **141**/full **244**/:app **184**/:core **343**+build+assembleDebug。走查 12 张 headless 模拟器实拍（新代码本地夹具网关 18790，用后即 kill 零常驻；凭据三零）=acceptance/uxz2-walkthrough/（WALKTHROUGH.md 逐元素销账）。**APK 9b27d2b6f01d7f95b14ffa67c04ad3474a793abfd5262c3d4d541861f4869b37=最新待装件**（dist 根五件未动）。
...
> 2. **DS（DeepSeek Harness 侦察+observed 实装）**：真机只读侦察**发现 `~/.dsh` 实存 7 个真实会话**（`sessions/<projectKey>/session-<uuid>/session.jsonl.zstd`=拼接 zstd 帧容器〔首帧 header JSONL+事件批次，44 型权威事件词表〕+`storages/session_projcache.json`/`workspace.json` 纯 JSON——harness 已在本机运行过，推翻「无用户侧数据」旧结论）；deepseekProvider 骨架→**observed 实装**：listSessions 走 projcache（零解压）/readMessages 走 node:zlib 内建 zstd（零新依赖零子进程）/startMonitor watch+seq 增量+状态判定；残尾帧跳过+未知事件容忍+损坏容器结构化降级；控制面恒未接（**managed 解除条件=ACP/SDK 通道〔packages/acp/acp+sdk/dsh-sdk-* 源码在位〕真机验证，验证须启动 harness=红线禁，待主控/用户裁决是否开运行验证批**）；dsobs-1..3 夹具用例（合成脱敏语料）。
> 3. **裁决候选留档**：①Kimi「发现型会话提升 managed」（网关 REST reply 对 kimi 发现型会话被既有 session_mode 门拒，docs/12 §5 语义，改动需裁决）；②DeepSeek managed 运行验证批（须授权启动 harness）；③docs/12 §8.5 骨架裁决节已过时（DS 走有源路径）可补 architecture 文档批。


> **✅ 「全做」mega-session（09-14 深夜，用户令「全做」）——八批全收（D5/W2/W3/X-U/X-L/X11/CERT/APK-F），main=99b5822 已推齐，门禁 typecheck 0+fast 125（新基线）/full 215/:app 146/:core 301/IPC 白名单 115**：
> 1. **D5 桌面收官（D-Aud 审计清单终清零，桌面优化线关账）**：I8 `dialog:pickPath` IPC（白名单 110→111，HandlerDeps 注入零 spawn，BackupPanel/Skills/Archive/MaterialImport 四处「浏览…」手输保留）/I11 快捷键（F5·Ctrl+R 刷新、Ctrl+1..9 切视图、斜杠聚焦搜索，输入态不抢占）/I12 App 级 toast 队列（16 处 per-panel useToast 迁移，CDP 双证 overlap 0 vs 旧实现 3480px²）/F6 relativeTime 全局 1min tick（format.ts 中文契约零触碰）/P3×4（A5 确认流统一——**实查 17 处 native confirm≠AUDIT 记 9，全迁 ConfirmDialog**，I5 直切裁决不回退/A6 中性空态图标/A7 transition .12s/A8 Versions 未检测禁 Update）；CDP 端到端（选择器四原生对话框 #32770 实开+深链 12/12+loadMore 307→507 回归全过）。
> 2. **W2 App 设备页**：relay 模式必 404 轮询退役（`shouldQueryServerRow` seam+「中继接入不提供设备列表查询」诚实文案行），直连零变化、撤销路径零触碰；:app 137。
> 3. **W3 token_rotation 文档批（裁决=甲档，编码不立项）**：实证 **C7a 已实现离线≤300s 补投闭环**（直投/pair 5s 冲刷窗/pendingRotations 内存补偿+grace 300s viaGrace）；真缺口=relay 重启丢内存补投表（四条件交叠概率极低+重配对兜底，接受现状）；**README 偏离单 #11「不补投」作废更正+docs/18 §3.14.1 正式入册**（单槽覆盖/绝不越窗/寿命单源时钟/三防线/重启缺口为已知边界）。
> 4. **X-U electron-updater 启用**：ECS generic feed 常量 `DEVHUB_UPDATE_FEED_URL='https://59.110.149.11/updates/'`+updateCenter 八值状态机（dev 门禁用/autoDownload=false/静默至多一次/安装门仅 downloaded）+`updates:status/check/download/install` 四通道（111→115）+设置卡（Agents 视图设置分组区）+`scripts/build-updates-feed.mjs`（**latest.yml path 连字符 vs 实际空格文件名差异实证+归一化装配+sha512/size 断言**——HANDOFF 旧注记就此闭环）+启动 60s 静默检查仅提示绝不自动下载。
> 5. **X-L 本地命令协议方案②（T1 本地模式结构性 Queued 根治）**：docs/18 §5.3.2 本地 workspace_link 请求面（零新 REST+三红线：token_rotation/event 帧绝不夹带本地/链来源=磁盘三文件重建绝不伪造/本地零凭据存储）+ws.ts 路由复用 Z3 资产（beginWorkspaceLink source 可选参缺省 relay-command，**relay 面逐字节不变**）+App 本地帧解析器结算+**U5 反转**（WorkspaceLinkModePolicy local 否定门→LocalGatewayFlow+三入口文案更新+10 单测改写）；**附带必要修复 openLocalWebSocket 丢弃 newWebSocket 返回值**（local 面 webSocket 句柄恒 null=sync/ack 出帧结构性失活根因）；ws 帧级端到端实证（ack accepted+result executed+链 URL 六参数结构+审计/日志零 URL 子串）。
> 6. **X11 收官重打包+换装**：B4 产物 asar 逐条验证（D5/X-U/X-L 增量+回归点）+常驻换装（**X11 版 PID 19228 在役**，health×3）+ECS caddy `/updates/` feed 部署（**Caddyfile 纯插 8 行 handle_path+file_server，validate+reload 零断流**，relay ws 重连 2s；三件 sha256 相符、目录 404 零索引）；**updater 互操作修复**（electron-updater@6.6.4 autoUpdater 惰性 getter 打包实例命名导出探测失败→命名直取+default 回退双形态）；静默检查已触发但撞 `net::ERR_CERT_AUTHORITY_INVALID`（自签 IP 证书不被 Chromium 信任→CERT 批靶心）。
> 7. **CERT 证书轮换双指纹（Phase A 侦察→主控裁决 D1-D5 甲档→Phase B 落地）**：实证（**App `gateway_config.pinFingerprints` 多值字段五层早已就绪零代码**〔存储/录入 UI/保存门/连接层/单测，指纹逗号换行分隔〕/桌面 host-leg 注入式双保险（fingerprints 多行+ca.pem 拼接，热装载 ≤60s）/**唯一缺口=updater 全流量走 `session.fromPartition("electron-updater")` 分区 session 非 defaultSession**〔6.6.4 源码取证纠正任务书候选解〕/**ECS 私有 CA 与叶同日 2026-12-04 到期**）；裁决=新 key 开窗（D1）/全新 CA 5y+短叶（D2=C）/仅分区 session 设 proc（D3）/P-256 维持（D4）/叶 180d（D5）；Phase B=`feedTrustProc`（叶 SPKI pin+有效期复核+**fail-closed**+hostname 分流零干预其他流量，pin 源=host-leg fingerprints 同文件热装载，dev 态不装）+**ECS /root/cert-rotation-staging 新 CA（5y→2031）+新叶（180d→2027-03-13）暂存，在役零触碰**（新叶 SPKI `sha256/b18a8433…`=base64 `sYqEM0IJipF86ZVti9aAHtz3aXLKpuR6aZzhu5aKZvw=`）+docs/23 全档（§6 裁决/§7 实施/暂存清单 SHA256SUMS）。**排程：M2 发布换装→M3 ≤11-20 开窗（用户在 App 高级字段追加新指纹）→M4 ≤12-01 轮换（caddy reload，分钟级回滚预案在 docs/23 §3.4）→M5 观察 ≥1 周收敛；12-04 19:28 GMT 过期=全 TLS 面拒的兜底红线**。
> 8. **APK-F 最终待装件**：`dist/DevHub-Android-0.1.0-debug.apk` sha256 `cd6d8caf…`（含 W2+X-L 全部安卓改动；:app 146/:core 301 复跑全绿）。

> **✅ RemoteWake 真关机实测已完成（09-14 晚，用户装 cd6d8caf 后实测）——链路全通、包已到网卡、S5 未醒=BIOS 层待用户排查**：新 APK 不闪退（P0 修复实证）；18:00-18:01 三次唤醒帧全 `sent`（ECS relay_audit #1430/#1431/#1435，exit 0，544-883ms）；Pi eth0=192.168.137.50/24 ICS 网段、广播路由正确、ping+ARP 对 192.168.137.1=b0:82:e2:4b:1a:81 REACHABLE（L2 实通）；本机 ASUS ROG STRIX B850-G（BIOS 1654/2026-04-02）：I226-V「唤醒魔包/唤醒模式匹配/S5 魔包唤醒」全启用+MSPower_DeviceEnable=True+非快速启动真 S5——**软件链零缺陷，剩余唯一嫌疑=BIOS APM 层（ErP Ready / Power On By PCI-E）**，待用户进 BIOS 核对（S3 睡眠唤醒为差分测试）。注：Get-NetAdapterPowerManagement 对 WLAN/I226 均报错 31=非管理员读 PM WMI 的固有失败，非驱动故障（勿据此误判）。

> **⚠️ 本会话主控事故两笔（如实入册）**：①`rm` 失败中断 `&&` 链致 **X-U 漏合 main**（fast 117≠121 是唯一破绽，X11 批 agent 抓出后批内并回）——教训：merge/push 等关键步勿与可失败清理命令同链；②`| tail` 管道吞 typecheck 退出码致 **ec97530 带类型错误推 main**（互操作 any 链 TS7006×3 环，两轮补丁 7023c95/e98db13 修复：.catch/.then/事件回调全量显式注解+**双态编译验证**不再依赖 node_modules 类型状态+主仓 `npm install` 环境补齐 lockfile 零变更）——现 typecheck 0（守门写法改 `if npm run typecheck; then`）。
> **遗留待清/待验**：句柄锁定残留目录 `.cdp-dist/`、`worktrees/x11-repackage/`、`worktrees/cert-dual/`（重启后可删，git 元数据已净）；**updater 下载→quitAndInstall 全链留 v0.1.1 真实发布时验证**（feed/检查/三态判定已在役）；electron-updater 分区名按锁版 6.6.4 锚定（升版须复核，失配=fail-closed 可诊断）；打包 publish 配置仍走 CLI 覆盖（`-c.publish.provider=generic`），正式入 package.json 待下次版本批。
...
> **✅ D2 已合 main（c458d47）+X9 终换装在役（09-14 01:46 PID 33808 health×3，asar 11/11 全命中）**：D2 用户面文案统一中文（27 文件+589/−486 纯展示层；术语表入 AUDIT 附注 A；**主控裁决落地 known-limitations §6.1 目标语言=中文为主技术名词英文，用户可翻回**；smoke 断言联动 0 处=format.ts relativeTime 中文契约零触碰原样过）；X9 包含 D1+D3+B3+D2 全量。门禁全程 typecheck 0+fast 115+full 209 各批独立复跑。
> **留档后备**：D-Aud P2×10/P3×10 未修清单（AUDIT.md）；本地命令协议方案②；桌面文案若用户要英文界面可翻回（纯文案易逆）。
...
> 3. **X6 已换装在役**（21:43 PID 11548 health×3 过；asar 9/9 全命中：B2 四修+全部回归点；dist 根五件 Setup sha256 158d743d）。**APK 6fcd774f=最新待装件**（含 P0 闪退修复+B1 五崩溃面修复）。全段推送已齐（a4a7052）。
...
> 2. **U4 回归实证 PASS/FAIL 各半（14c4dd2，证据 19 截图）**：横屏修复 PASS（父/子双形态实证）+markdown 实况 PASS（表格未遇注记）+徽章/弹层留档 PASS；**FAIL=T1 卡本地模式结构性 Queued**（真发现）。
> 3. **Z3 侦察结论 B**：本地模式缺双端命令面（App 本地帧解析器无 command 结算+网关无路由；token_rotation/event 帧夹带全撞红线）——主控裁决方案①诚实文案（本地模式=模拟器/同机专用，不值当扩本地命令协议；可复用资产 beginWorkspaceLink/completeWorkspaceLink/buildZcodeWorkspaceLinkDefault 已登记供未来方案②）。
> 4. **U5 已合 main（887650a）**：WorkspaceLinkModePolicy 三分口径（local 不取链+三入口「本地模式不提供 ZCode 遥控取链 · 请使用 Relay 接入」/relay 逐字节不变自证）+请求模式门；新增 10 单测 :app **121**/:core **301**；**APK 0423ad08=最新待装件**（00:02）。常驻 X5 在役（桌面零改动免换装）。
> 5. **体验优化线终态**：审计清单（P0~P3）全部消费完毕；遗留候选仅=本地命令协议全量方案②（需时再立项）+横屏回归已实证闭环。
...
> 3. **X5 已换装在役**（21:24 PID 53900 health×3 过；asar 十项：evalZcodeTurnStatus/turn_usage 在位+回归点全命中；dist 根五件 Setup sha256 eb21b4dd；**APK 47158b8f=最新待装件**）。体验优化线仅剩候选=M1 横屏旋转回归实证（待桌面 ZCode 链路就绪自然时机）。主仓完整深扫 findingCount=0（封印 03ea203a，09-12 22:52）——「扫描覆盖不完整」警告闭环。**U4 回归实证批执行中**（横屏 26 号场景+T1 Ready 态+markdown 实况；桌面 ZCode 链路已实测存活：今日 277 条 web-remote-control 行）。**U4 已收官（14c4dd2）**：横屏修复 PASS（父/子会话双形态三张齐实证）+markdown 实况 PASS（表格未遇如实注记）+徽章/弹层留档 PASS；**新发现=T1 卡本地模式结构性 Queued**（本地网关 ws.ts 对 workspace_link 帧静默忽略无 ack；relay 面用户真机不受影响）→**Z3 批执行中**（Phase1 侦察定形态：A 本地网关接线/B 停手转 App 诚实文案）。证据 19 截图入册。
