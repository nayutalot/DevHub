...
0. **「赛程钉 ContestPin」比赛模块已立项·开工中**（用户 2026-09-09 指令，当晚新会话已开工）：权威任务书=`docs/briefs/contestpin-charter.md`（逐字原文，32a484c）——八节全规格（管理/多节点/悬浮窗/提醒/两阶段识别/多模态/Agent/备份打包）。**进度（09-09 晚）**：三面只读审计完成 → CP0 文档批已合 main（9067e8e：docs/22 设计书+docs/03 008 预告【007 留 LR1】+docs/04 CP1 通道 70→79 预告+m1 任务书）→ **CP1 数据层批已完成并过主控 review**（agent/contestpin 分支 6 commits 22d023d..5c7395d 已推：008 迁移 7 表+9 通道 79+contestService+fast 用例；主控独立复跑 tsc 0 error+fast 85/85；**合 main 等窗毕全量门禁**）→ CP2 悬浮窗批派发。**⚠️ CP1 事故（已遏制待清理）**：既有用例 uxa-147 原误标 fast 档且硬编码 8746，CP1 三次 fast 验证各打一次真实网关（11:36/11:37/11:38Z 各一组 pairing create+claim+GET sessions），**真实库疑新增 3 条 `uxa-147-phone` 幻影设备行+审计日志**（无 managed spawn/命令提交/业务写）；档位已归位 fast→full（断言零改动），主控日志比对确认此后 fast 档零网关请求。**窗毕队列**：①M3-D 终报接收落档；②撤销 3 条幻影设备行（agents:deviceRevoke 语义）+审计日志核对；③全量门禁（tsc/smoke 全量/mcp/:core/:app/ecs-relay）→ 合 agent/contestpin → main；④CP2 悬浮窗 electron 实启动逐项验证（7 能力+托盘+导航+多显示器+退出清理）；⑤backlog：uxa-147/148 的 8746 硬编码与端口顺延脱钩（归原批次修）。**时机分流不变**：窗内只隔离编码+纯 Node 单测；与 M3-E1/LR1 三线并行按资源登记排队（当前仅 ContestPin 解锁）。**双会话防冲突**：M3-D 巡检纯观察哨，ContestPin 会话独占全部文件/HANDOFF/记忆同步职责。
