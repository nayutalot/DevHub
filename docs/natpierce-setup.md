...
> 本文是 AC8 批次新增的用户文档（docs/16 §1 AC8 行「NatPierce 隧道透传验证
> （用户自备凭据，外置配置）」的配套引导）。变量命名与三态投影行为以代码实况
> `src/main/services/agentControl/natpierce.ts` 为准（本批核对一致）。
...
- 一个可用的 NatPierce 服务端地址（公网可达的 endpoint URL）；
- 该服务上的账号（account）；
- 该服务签发的访问凭据（token）。

三者全部由你自己向 NatPierce 服务方获取，DevHub 不代理、不代办。

## 3. 环境变量清单（与 ac8-140 实现逐字一致）
...
三态投影（`agents:gatewayStatus` 的 `natpierce` 字段）：

- 三者全未配置 → `{ configured: false }`（与 AC2 契约形状逐字节一致）；
...
设置方式（Windows，用户级永久环境变量；设置后重启 DevHub 生效）：
...
1. 桌面端开启 Remote Gateway：Agents 视图 → 网关设置 → 启用（settings
   `gateway_enabled=1`；默认端口 8746，可用 `gateway_port` 改）。
2. NatPierce 客户端添加一条 **TCP/HTTP 透传规则**：本地地址 `127.0.0.1`，本地
   端口 `8746`，映射到你的服务端域名/端口（例如 `https://np.example.com`）。
3. 手机的 App → 网关配置：主机填你的隧道地址对应的主机名（**不能**再填
   `10.0.2.2`/`127.0.0.1`——那分别是模拟器回环映射和手机本机），端口填隧道对
   外端口；保存并连接。若此前已配对，直接重连即可；未配对走 8 位码配对。
...
- **App 侧仍需配对**：隧道打通 ≠ 能连上。手机必须持有效设备 Token（8 位码
  配对签发，桌面只存哈希）；未配对设备在鉴权层即被拒（401）。撤销设备
...
- **Android 模拟器**：App 默认 `10.0.2.2:8746`（模拟器回环映射到宿主机
  `127.0.0.1`）——AC7/AC8 批次的真机验收即走此路径，无需任何隧道。
