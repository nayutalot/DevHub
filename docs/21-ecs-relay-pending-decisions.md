...
- **终局（2026-09-10）**：① ✅（M3-D 终报 240/240 零故障）；② 部分——R-B8 四断言全过、
  R-B5 腿1 过、腿2「真实推理回流」因 codex 上游限流 429 未达，**用户裁决烂尾销账并豁免
  直接发布**（「直接release」「codex 不用管了，当烂尾了」）；③ ✅；④ ✅ **v0.1.0 已发布**
  （2026-09-10 16:5x，tag `v0.1.0`→37beb5c，108 通道构建，资产三件核验过；正文门禁数字
  已校正为发布物实况 192/108）。
