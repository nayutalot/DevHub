# M3 集成联调与验收 —— 批次规划（v2 定稿，2026-09-06；M2 已收官，基线数字已核实）

> v1→v2：填入 M2 收官实测基线（smoke 164/:core 167）；新增 M3-C 验收面一条（R3 信标遗留，见 §5）。
...
- **任务**：`scripts/smoke.mjs` 拆两档——**快速档**（单元/纯逻辑用例，目标 <2min，每批跑）+ **全量档**（Gateway/e2e/端口类，合并时跑）。**164 基线（M2 收官实测）一条不删**（append-only 铁律）；分层=用例标记/分组，不改用例本体。
...
- **R3 信标遗留（M2-R3 收尾批如实申报的偏差，本批补齐）**：`upstream:disconnected` 真信标徽标的端到端截图——M2 批内禁止任何监听无法承载 relay 桩，已交付不可达降级态（relay-03/03b）；M3-C 真实 wss 链路窗口内补「已连 relay→upstream 断」的信标帧证据。
