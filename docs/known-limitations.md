...
### 5.2 无域名 IP TLS 的信任面限制（2026-09-05 U1 裁决引入，docs/21 §1）

- **自签 IP 证书不受浏览器/系统默认信任**：443 终结自签证书（SAN 含
  `IP:59.110.149.11`），直接用浏览器访问 `https://59.110.149.11` **必出安全告警——
  属预期行为，不是缺陷**；任何口径绝不声称浏览器默认信任该证书。受支持的 TLS
  消费者只有三类且全部覆盖：Android App（OkHttp `CertificatePinner`，注入式
  `TlsPinningConfig`）、Windows relayClient（Node `tls.checkServerIdentity` 覆写
  + CA 指纹校验）、运维调试（`curl --cacert` 显式指定自签 CA）。
- **证书轮换需双指纹窗口 + 重分发指纹配置**：轮换期客户端同时配置旧+新两枚
  SPKI 指纹（任一匹配即信任，docs/19 §10.4），轮换完成后重分发仅含新指纹的
  配置并收敛回单指纹；指纹配置是数据不是代码，不依赖 App 发版，但**重分发
  本身是人工动作**——漏分发旧指纹/新指纹任一侧都会断链（TLS 三拒中的错误指纹
  拒绝是设计行为）。
- **明文时间盒限制**：`ws://` 明文禁止作为正式方案，仅限 M3 联调窗口内的临时
  联调且不承载真实配对（配对码/端到端 Token/Relay 凭据明文绝不走明文链路，
  docs/19 §11 精确过期条件）；时间盒过期后明文入口按事故处理。
- **90 天证书有效期**：自签证书有效期 90 天（`gen-ip-cert.sh` 内置续期提示），
  过期即全链路 TLS 握手失败（三拒之「过期证书被拒」）——到期未续期 = 443 断链，
  需纳入部署运维清单。
